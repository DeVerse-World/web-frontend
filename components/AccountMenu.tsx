...
export default function AccountMenu(props: DOMAttributes) {
  ...
  useEffect(() => {
    if (remoteConfig != null)
      FirebaseService.getShouldShowDashboardToggle(remoteConfig).then(
        setShowDashboardToggle
      );
  }, [remoteConfig]);
      ...
      <Menu as="div" className="relative">
        ...
        >
          <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-dark py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <Menu.Item>
              {({ active }) => (
                <Link href="/creator-about" prefetch={false}>
                  <a
                    className={classNames(
                      active ? "bg-medium text-lightest" : "",
                      "block px-4 py-2 text-sm text-light",
                      "no-underline"
                    )}
                  >
                    Profile
