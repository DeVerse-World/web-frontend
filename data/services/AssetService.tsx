...
class AssetService extends BaseService {
  ...
  assetContract: Contract = new ethers.Contract(
    assetAddress,
    AssetABI.abi,
    new ethers.providers.JsonRpcProvider(process.env.RPC_URL)
  );
  transferSingleEventFilter: ethers.EventFilter = {};
  // this.assetContract.filters.TransferSingle();
  ...
  async uploadAsset(file: File, onProgress: (number) => void): Promise<string> {
    console.log("FILE\n");
    console.log(file);
    let res = await this._ipfsClient.add(file, {
      progress: (progress, path) => {
        onProgress(Math.round((progress / file.size) * 100));
      },
    });
  ...
  async createAsset(preAsset: NFTAsset): Promise<string> {
    ...
    let ipfsHashString =
      "0x" +
      CIDTool.format(CIDTool.base32(res.path), { base: "base16" })
        .toString()
        .slice(9);
  ...
  async _enrichOpenseaFields(asset: NFTAsset): Promise<NFTAsset> {
    ...
    if (asset.animation_url != "") {
      asset.animation_url = await this._cidFullV0ToFullV1Base32(
        asset.animation_url
      );
    }
    return asset;
  ...
  async mint(apiStrategy: ApiStrategy, ipfsHashString, supply) {
    ...
    switch (apiStrategy) {
      case ApiStrategy.REST:
        let transaction = await assetContract.mint(
          creator,
          packId,
          hash,
          supply,
  ...
  async getAll(
    ...
    switch (apiStrategy) {
      case ApiStrategy.REST:
        let transferEvents = await this.assetContract.queryFilter(
          this.transferSingleEventFilter
        );
        transferEvents.forEach((transferEvent) => {
          parallelJobs.push(
            new Promise<NFTAsset>(async (resolve, reject) => {
      ...
      case ApiStrategy.GraphQl:
        ...
        assetTokens.forEach((token) => {
          parallelJobs.push(
            new Promise<NFTAsset>(async (resolve, reject) => {
              this._getAssetFromEther(token.tokenURI)
                .then((res) => {
                  if (res.isFailure()) {
                    reject(res.error);
                  } else {
  ...
  async _getAssetFromEther(tokenFullUri: string): Promise<Result<NFTAsset>> {
    ...
    asset.fileAssetUri = asset.fileAssetUri.replace(
      this._oldUriPrefix,
      this._uriPrefix
    );
    asset.file2dUri = asset.file2dUri.replace(
      this._oldUriPrefix,
      this._uriPrefix
    );
    asset.file3dUri = asset.file3dUri.replace(
      this._oldUriPrefix,
      this._uriPrefix
    );
    ...
    deverseClient({
      ...
      data: {
        minted_nft: {
          ...
          supply: asset.supply,
          file_asset_uri: asset.fileAssetUri,
          file_asset_name: asset.fileAssetName,
          file_asset_uri_from_centralized: asset.fileAssetUriFromCentralized,
          file_2d_uri: asset.file2dUri,
          file_3d_uri: asset.file3dUri,
